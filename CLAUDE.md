...
`-c` is an alias for `--config`; the CLI flag wins over `DUOCB_CONFIG`. Without an override, both processes resolve to the same default location (see README) and collide. Joining is by choosing Join on the home hub, which opens the device picker, and selecting the peer there (any listed device — the join retries with backoff for up to 10 initial attempts, so press Join again if the host starts later); no peer name is typed. Configs are per-machine, never copied between machines.
