...
- Offline LAN pairing relies on mDNS being usable on the local network segment. LAN-only PIN is the strict no-internet mode; manual mode can also work without internet on a LAN but retains the normal relay/DNS configuration.
