...
duocb is a two-device P2P clipboard-sharing desktop app: a **Slint** front end over a **tokio** networking runtime that pairs two iroh endpoints (QUIC/TLS 1.3), authenticates in-band, and then pumps clipboard items over one long-lived bidirectional stream. Signaling (how the client learns the server's ephemeral node id) is either **nostr** (token-derived or PIN-derived rendezvous records) or **local-network PIN discovery** — mDNS/DNS-SD multicast, or, where multicast is blocked, a **unicast side channel** the joiner reaches by typing the host's LAN IP. Both carry the same PIN-encrypted node-id record.
...
1. Resolve the target **each attempt**: configure mode = look up the **selected peer's** presence record by its exact identity tag and dial the node id inside (so a restarted host with a fresh node id self-heals; a missing hosting id is retried and counts toward the attempt bound); PIN mode = rendezvous lookup on the selected internet/LAN channels (a LAN-only PIN with a typed host IP fetches the record from that host's unicast side channel instead of browsing mDNS), with a **pinned node id fast path** after the first pairing (the PIN has rotated out of discovery, but the server retains our pairing key, so reconnects dial the remembered id and re-prove the same PIN in-band).
...
The LAN-only channel advertises the encrypted record as a spec-compliant Bonjour/DNS-SD service (`_duocb-pin._udp`, `crate::lan::dnssd`) whose SRV/A/AAAA records carry the host's direct addresses; the joiner dials those as resolved. Where multicast is blocked, the same host *also* serves the record over a **unicast side channel** (`crate::lan::unicast`): a small TCP listener on a PIN-derived port (`pin::side_channel_port` maps the PIN into the ephemeral range, so different PINs coexist and no port is ever typed) that returns the same NIP-44 ciphertext plus the host's direct socket addresses. A joiner who types the host's LAN IP fetches that record directly (no multicast), decrypts it with the PIN, and dials the returned addresses — otherwise it browses mDNS. The presence of a typed IP is the only thing that selects the path; the PIN is identical either way and the in-band PIN auth is unchanged, so the side channel only bootstraps discovery (like mDNS) and a spoofed node id still cannot pass auth. The listener follows the rotating PIN with a one-period look-back (mirroring the mDNS advert window) and is withdrawn once a peer pairs.
...
| token fingerprint | SHA-256 (first 8 bytes) | token string | 16 uppercase hex digits, grouped `XXXX-XXXX-XXXX-XXXX`, display only |
