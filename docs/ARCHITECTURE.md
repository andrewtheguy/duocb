...
- LAN-only PIN pairing relies on mDNS being usable on the local network segment. Its traffic is always direct and never uses a middle server, but LAN subnet membership is not enforced at the packet level. Manual mode can also work without internet on a LAN but retains the normal relay/DNS configuration.
