...
#
# The iOS build differs from the desktop one in exactly one place: iroh's own
# mDNS address lookup is compiled out, because it opens multicast sockets
# in-process and iOS gates those behind an entitlement Apple grants by
# exception. Regular mDNS still works — duocb-core's DNS-SD backend talks to
# the system mDNSResponder daemon instead. See crates/duocb-core/src/lan/dnssd.rs
# and net/endpoint.rs.
#
# Usage:
...
# (../duocb-ios targets iOS 17). Override via env.
