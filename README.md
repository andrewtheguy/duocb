...
- **Works offline on a LAN** — node ids resolve via mDNS with zero internet (today through the manual testing mode; LAN support for PIN quick pair is planned)
...
**Quick options:** press `Q` on both devices (PIN is preselected; `M` switches to the manual testing mode), press `S` on the starting device and `C` on the joining one, and type the displayed credentials.
