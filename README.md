...
3. **Pair:** press `S` (Start) on one device; on the other press `C` (Join) to open the device picker, select it (`R` refreshes the list), and press `Enter`. Joining first and starting shortly afterward also works: the join retries with backoff for up to 10 attempts. If it gives up first, press Join again after the host starts.
