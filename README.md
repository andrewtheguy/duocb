...
- **Works offline on a LAN** — the PIN quick pair discovers peers over local mDNS (and node ids resolve the same way), so two devices on one network pair with zero internet
...
3. **Pair:** press `S` (Start) on one device; on the other press `C` (Join) to open the device picker, select it (`R` refreshes the list; a *hosting* badge shows who already started), and press `Enter`. Joining first and starting shortly afterward also works: the join retries with backoff for up to 10 attempts. If it gives up first, press Join again after the host starts.
...
| `Ctrl/⌘+T` | when available | copy the pairing code (manual host) or the secret (hub) |
