...
| **Manual** (testing only) | none — you carry the code | one pairing code (node id + session secret + direct addresses) | Argon2id session-secret challenge-response (mutual, in-band) | **not required** on the same LAN (the code embeds the addresses) |
...
- **Manual** is a testing-only option, not an everyday pairing path: it is the PIN flow with the discovery removed entirely — instead of any channel carrying the rendezvous record, *you* carry it. The starting device displays a single **pairing code** (its node id, a fresh session secret, and its direct socket addresses, checksummed and with a **Copy** action); transfer it over any channel you trust and paste it on the joining device, which dials the embedded addresses (and node id) and proves the secret with the same in-band Argon2id challenge-response the PIN modes use. That makes it the way to verify connectivity between two devices with zero signaling and zero discovery — offline on the same LAN even where mDNS/multicast is blocked, or across the internet on different networks. The code stays valid for the whole session, so the paired peer can reconnect after a drop.
